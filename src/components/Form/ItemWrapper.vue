...
const updateChildState = () => {
  ...
  if (slotElements.length > 0) {
    ...
    childIsValid.value = Array.from(slotElements).every(el => el.getAttribute('data-isvalid') === 'true')
  }
...
onMounted(() => {
  ...
  observer.observe(document.body, { childList: true, subtree: true, attributes: true, attributeFilter: ['data-isvalid', 'data-form-type'] })
...
<template>
  <div ref="rootElement" :data-child-isvalid="childIsValid" :data-child-form-type="childFormTypes.join(' ')">
    <slot />
  </div>
</template>